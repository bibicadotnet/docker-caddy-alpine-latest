...
RUN echo "Cài đặt các gói cần thiết..." && \
    apk add --no-cache \
        ca-certificates \
        libcap \
        mailcap \
...
RUN echo "Tạo các thư mục cần thiết..." && \
    mkdir -p \
        /config/caddy \
        /data/caddy \
        /etc/caddy \
...
RUN echo "Tải Caddyfile và trang chào mừng mặc định..." && \
...
RUN echo "Lấy phiên bản mới nhất của Caddy từ GitHub API..." && \
...
RUN echo "Kiến trúc hệ thống: $(apk --print-arch)" || { echo "Lỗi: Không thể xác định kiến trúc hệ thống"; exit 1; }

# Tải và cài đặt Caddy
RUN echo "Tải và cài đặt Caddy..." && \
    set -eux; \
